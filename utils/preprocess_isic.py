...
for path in train_img_path:
    ...
    if os.path.exists(mask_path):
        cmd = f'cp {path} {os.path.join(outdir,"traindataset","masks")}'
        os.system(cmd)
    else:
...
for path in val_img_path:
    ...
    mask_path = os.path.join("data/ISIC2018_Task1/traindataset/masks", mask_file)
    if os.path.exists(mask_path):
        cmd = f'cp {path} {os.path.join(outdir,"valdataset","masks")}'
        os.system(cmd)
    else:
        print("not exist {}, check again".format(mask_path))
...
for path in test_img_path:
    ...
    mask_path = os.path.join("data/ISIC2018_Task1/traindataset/masks", mask_file)
    if os.path.exists(mask_path):
        cmd = f'cp {path} {os.path.join(outdir,"testdataset","masks")}'
        os.system(cmd)
    else:
        print("not exist {}, check again".format(mask_path))