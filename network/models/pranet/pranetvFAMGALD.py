...
class FAM(nn.Module):
    ...
    def forward(self, left, down, right, crop):
        ...
        x = -1 * (torch.sigmoid(crop)) + 1  # [bs, 1, 11, 11]
        x = x.expand(-1, self.in_channel_down, -1, -1)  # [bs, in_channel_down, 11, 11]
        down = x.mul(down)
...
class PraNetvFAMGALD(nn.Module):
    # res2net based encoder decoder
    def __init__(self, channel=32):
        super(PraNetvFAMGALD, self).__init__()
        # ---- ResNet Backbone ----
        print("PraNetvFAMGALD")
    ...
    def forward(self, x):
        ...
        crop_5 = F.interpolate(ra5_feat, scale_factor=0.25, mode="bilinear")
        ...
        # crop_2 = F.interpolate(crop_3, scale_factor=2, mode="bilinear")
