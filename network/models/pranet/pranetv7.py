...
from ...contextagg import GALDHead, DualGCNHead


class PraNetv7(nn.Module):
    # res2net based encoder decoder
    def __init__(self, channel=32):
        super(PraNetv7, self).__init__()
        # ---- ResNet Backbone ----
        print("PraNetv7")
    ...
    def forward(self, x):
        ...
        x_head_gald_out = F.interpolate(x_head_gald,
                                        scale_factor=16,
                                        mode='bilinear')
        x_head_dual = self.head_dual(x4)
        # print(x_head_dual.shape,"x_head_dual")
        x_head_dual_out = F.interpolate(x_head_dual,
                                        scale_factor=32,
                                        mode='bilinear')
