...
from .gcpa_gald import FAM, FAMAG, CA, SA
from ...encoders import res2net50_v1b_26w_4s, hardnet
from .gcpanet import ResNet


class GCPACCv2Net(nn.Module):
    def __init__(self):
        super(GCPACCv2Net, self).__init__()
        ...
        self.long_relation_b = CrissCrossAttention(interplanes)
    ...
    def forward(self, x):
        ...
        out5_ = self.convb(out5)  # bs, 256, 11, 11
        out5_c = self.long_relation_b(out5_)  # bs, 256, 11, 11
        out5 = self.local_attention_b(out5_c)  # bs, 256, 11, 11

        # out5_a = self.sa55(out5, out5)
        # out5 = self.ca55(out5_a, out5)  # bs, 256, 11, 11
