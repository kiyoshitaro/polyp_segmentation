...
def model_hub(arch, pretrained=True, nl_type=None, nl_nums=None, pool_size=7):
    ...
    if arch == "50":
        return resnet50(
            pretrained=pretrained, nl_type=nl_type, nl_nums=nl_nums, pool_size=pool_size
        )
    elif arch == "101":
        return resnet101(
            pretrained=pretrained, nl_type=nl_type, nl_nums=nl_nums, pool_size=pool_size
        ...
        raise NameError(
            "The arch '{}' is not supported yet in this repo. \
                You can add it by yourself.".format(
                arch
            )
        )
...
class ResNet(nn.Module):
    def __init__(
        self, block, layers, num_classes=1000, nl_type=None, nl_nums=None, pool_size=7
    ):
        self.inplanes = 64
        ...
        if not nl_nums:
            ...
            self.layer3 = self._make_layer(
                block, 256, layers[2], stride=2, nl_type=nl_type, nl_nums=nl_nums
            )
    ...
    def _make_layer(self, block, planes, blocks, stride=1, nl_type=None, nl_nums=None):
        ...
        for i in range(1, blocks):
            if (
                (i == 5 and blocks == 6)
                or (i == 22 and blocks == 23)
                or (i == 35 and blocks == 36)
            ):
                if nl_type == "nl":
                    layers.append(
                        SpatialNL(self.inplanes, int(self.inplanes / 2), use_scale=True)
                    )
                elif nl_type == "cgnl":
                    layers.append(
                        SpatialCGNL(
...
def load_partial_weight(model, pretrained, nl_nums, nl_layer_id):
    ...
    for k, v in _pretrained.items():
        ...
        if nl_nums == 1 and layer_name == "layer3.{}".format(nl_layer_id):
            ks[1] = str(int(ks[1]) + 1)
            k = ".".join(ks)
...
def resnet50(pretrained=False, nl_type=None, nl_nums=None, **kwargs):
    ...
    model = ResNet(Bottleneck, [3, 4, 6, 3], nl_type=nl_type, nl_nums=nl_nums, **kwargs)
    if pretrained:
...
def resnet101(pretrained=False, nl_type=None, nl_nums=None, **kwargs):
    ...
    model = ResNet(
        Bottleneck, [3, 4, 23, 3], nl_type=nl_type, nl_nums=nl_nums, **kwargs
    )
    if pretrained:
...
def resnet152(pretrained=False, nl_type=None, nl_nums=None, **kwargs):
    ...
    model = ResNet(
        Bottleneck, [3, 8, 36, 3], nl_type=nl_type, nl_nums=nl_nums, **kwargs
    )
    if pretrained:
