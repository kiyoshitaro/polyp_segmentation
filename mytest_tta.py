...
def flip_lr(images):
    return torch.from_numpy(np.flip(images.cpu().detach().numpy(), axis=2).copy()).cuda()

def shift(images, shift, axis):
    return torch.from_numpy(np.roll(images.cpu().detach().numpy(), shift, axis=axis)).cuda()

def rotate(images, angle):
    return torch.from_numpy(ndimage.rotate(
        images.cpu().detach().numpy(), angle, axes=(1,2),
        reshape=False, mode='nearest')).cuda()
    ...
    for id in list(folds.keys()):
        ...
        for e in epochs:
            ...
            for i, pack in tqdm.tqdm(enumerate(test_loader, start=1)):
                ...
                # sys.exit()
                res = F.upsample(
                    pred, size=gt.shape, mode="bilinear", align_corners=False
                )
                res = res.sigmoid().data.cpu().numpy().squeeze()
                pred = (res - res.min()) / (res.max() - res.min() + 1e-8)


                _,_,_ ,x =  model(flip_lr(image))
                pred_f = flip_lr(x)
                res = F.upsample(
                    pred_f, size=gt.shape, mode="bilinear", align_corners=False
                )
                res = res.sigmoid().data.cpu().numpy().squeeze()
                pred_f = (res - res.min()) / (res.max() - res.min() + 1e-8)

                _,_,_ ,x =  model(shift(image, -3, axis=2))
                pred_w0 = shift(x, -3, axis=2)
                res = F.upsample(
                    pred_w0, size=gt.shape, mode="bilinear", align_corners=False
                )
                res = res.sigmoid().data.cpu().numpy().squeeze()
                pred_w0 = (res - res.min()) / (res.max() - res.min() + 1e-8)

                _,_,_,x = model(shift(image, 3, axis=2))
                pred_w1 = shift(x, -3, axis=2)
                res = F.upsample(
                    pred_w1, size=gt.shape, mode="bilinear", align_corners=False
                )
                res = res.sigmoid().data.cpu().numpy().squeeze()
                pred_w1 = (res - res.min()) / (res.max() - res.min() + 1e-8)


                _,_,_,x = model(shift(image, -3, axis=1))
                pred_h0 = shift(x,3, axis=1)
                res = F.upsample(
                    pred_h0, size=gt.shape, mode="bilinear", align_corners=False
                )
                res = res.sigmoid().data.cpu().numpy().squeeze()
                pred_h0 = (res - res.min()) / (res.max() - res.min() + 1e-8)


                _,_,_,x = model(shift(image, 3, axis=1))
                pred_h1 = shift(x,-3, axis=1)
                res = F.upsample(
                    pred_h1, size=gt.shape, mode="bilinear", align_corners=False
                )
                res = res.sigmoid().data.cpu().numpy().squeeze()
                pred_h1 = (res - res.min()) / (res.max() - res.min() + 1e-8)


                _,_,_,x = model(rotate(image, -90))
                pred_r0 = rotate(x,90)
                res = F.upsample(
                    pred_r0, size=gt.shape, mode="bilinear", align_corners=False
                )
                res = res.sigmoid().data.cpu().numpy().squeeze()
                pred_r0 = (res - res.min()) / (res.max() - res.min() + 1e-8)


                _,_,_,x = model(rotate(image, 90))
                pred_r1 = rotate(x,-90)
                res = F.upsample(
                    pred_r1, size=gt.shape, mode="bilinear", align_corners=False
                )
                res = res.sigmoid().data.cpu().numpy().squeeze()
                pred_r1 = (res - res.min()) / (res.max() - res.min() + 1e-8)



                # preds_f = np.stack((pred, pred_f))
                # res = np.mean(preds_f, axis=0)

                # preds_w = np.stack((pred, pred_w0, pred_w1))
                # res = np.mean(preds_w, axis=0)

                # preds_h = np.stack((pred, pred_h0, pred_h1))
                # res = np.mean(preds_h, axis=0)

                # preds_hw = np.stack((pred, pred_h0, pred_h1, pred_w0, pred_w1))
                # res = np.mean(preds_hw, axis=0)

                # preds_fhw = np.stack((pred, pred_h0, pred_h1, pred_w0, pred_w1, pred_f))
                # res = np.mean(preds_fhw, axis=0)

                # preds_r = np.stack((pred, pred_r0, pred_r1))
                # res = np.mean(preds_r, axis=0)

                # preds_fhwr = np.stack((pred, pred_h0, pred_h1, pred_w0, pred_w1, pred_f, pred_r0, pred_r1))
                preds_fhwr = np.stack((pred,pred_r0, pred_r1))
                res = np.mean(preds_fhwr, axis=0)
